...
// Tổng số user (trừ admin)
$total_sql = "SELECT COUNT(*) AS total FROM users WHERE role != 'admin'";
$total_users = $conn->query($total_sql)->fetch_assoc()['total'];
$total_pages = ceil($total_users / $limit);

// Lấy danh sách user theo trang
$sql = "SELECT user_id, user_name, user_email FROM users WHERE role != 'admin' LIMIT ? OFFSET ?";
