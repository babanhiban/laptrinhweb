...
SELECT orders.order_id, order_details.product_id FROM orders 
JOIN order_details ON orders.order_id = order_details.order_id;

-- 4. Liệt kê thông tin mua hàng của người dùng
SELECT users.user_id, users.user_name, orders.order_id, order_details.product_id 
FROM users 
JOIN orders ON users.user_id = orders.user_id 
JOIN order_details ON orders.order_id = order_details.order_id;
...
SELECT users.user_id, users.user_name, orders.order_id, order_details.product_id, products.product_name 
FROM users 
JOIN orders ON users.user_id = orders.user_id 
JOIN order_details ON orders.order_id = order_details.order_id 
...
-- 8. Liệt kê đơn hàng có giá cao nhất của mỗi user
...
-- 9. Liệt kê đơn hàng có giá thấp nhất của mỗi user
SELECT user_id, user_name, order_id, total_price FROM (
    SELECT users.user_id, users.user_name, orders.order_id, SUM(products.product_price) AS total_price, 
